...
struct scheduler {
 ...
 private:
  ...
  struct alignas(128) workerInfo {
    ...
    workerInfo(std::size_t worker_id_, scheduler* s) : 
      worker_id(worker_id_), my_scheduler(s),
      rng_seed(hash(time(0)) ^ hash(worker_id_ + 1)) {}
    ...
    workerInfo& operator=(workerInfo&& w) noexcept {
      if (this != &w) {
        ...
        rng_seed = std::exchange(w.rng_seed, 0);
      }
      return *this;
    }
 ...
 private:
  ...
  Job* steal_job(F&& break_early, bool timeout) {
    ...
    do {
      ...
      std::this_thread::sleep_for(std::chrono::nanoseconds(num_deques * SLEEP_FACTOR));
  ...
  Job* try_steal(size_t id) {
    ...
    size_t target;
    do {
      worker_info.rng_seed = hash(worker_info.rng_seed);
      target = (worker_info.rng_seed) % num_deques;
    } while (target == id);
